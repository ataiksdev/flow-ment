...
import { Plus, Check, Pencil, Trash2, X, Play } from "lucide-react";
...
function RoutineForm({ onSave, onCancel }: {
  ...
  const addBlock = () => setBlocks((b) => [...b, { label: "", categoryId: categories[0]?.id ?? 0, durationMinutes: 30 }]);
      ...
      <Drawer.Portal>
        ...
        <Drawer.Content className="bg-card flex flex-col mt-4 fixed bottom-0 left-0 right-0 z-50 pb-8 px-5 pt-4 border-t-2 border-border max-h-[92vh] overflow-y-auto">
          ...
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-xl font-bold uppercase tracking-tight">New Routine</h2>
            <button onClick={onCancel} className="w-9 h-9 border border-border flex items-center justify-center text-muted-foreground hover:bg-muted">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="space-y-2 mb-4">
            {blocks.map((bl, i) => (
              <div key={i} className="flex items-center gap-2 bg-background border border-border p-2">
                <div className="w-3 h-3 flex-shrink-0" style={{ backgroundColor: categories.find(c => c.id === bl.categoryId)?.color ?? "#aaa" }} />
                <input type="text" value={bl.label} onChange={(e) => updateBlock(i, "label", e.target.value)}
                ...
                <select value={bl.categoryId} onChange={(e) => updateBlock(i, "categoryId", Number(e.target.value))}
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  <input type="number" value={bl.durationMinutes} min={5} max={180} step={5}
                ...
                <button onClick={() => removeBlock(i)} className="w-6 h-6 flex items-center justify-center text-destructive/60 hover:text-destructive">
                  <X className="w-3 h-3" />
                </button>
              </div>
            ))}
          ...
          <button
            onClick={() => name.trim() && blocks.length > 0 && onSave({ name: name.trim(), color, blocks, createdAt: new Date().toISOString() })}
...
export default function Habits() {
  ...
  const last30Days = Array.from({ length: 30 }).map((_, i) => format(subDays(new Date(), 29 - i), "yyyy-MM-dd"));

  const getStreak = (habitId: number) => {
    let streak = 0;
    for (let i = 0; i < 30; i++) {
      const d = format(subDays(new Date(), i), "yyyy-MM-dd");
      if (allLogs.some((l) => l.habitId === habitId && l.date === d)) streak++;
      else if (i > 0) break;
    }
    return streak;
  };
  ...
  const applyRoutine = async (comp: Composition) => {
    const now = new Date();
    ...
    for (const block of comp.blocks) {
      ...
      await addTimeEntry({ description: block.label || comp.name, categoryId: block.categoryId, startTime: start.toISOString(), endTime: end.toISOString(), date: dateStr, type: "manual" });
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      <div className="flex items-center justify-between px-4 pt-6 pb-4 border-b-2 border-border flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-bold text-3xl uppercase tracking-tight">
            {tab === "habits" ? "Habits" : "Routines"}
          </h1>
        </div>
        <button
          onClick={() => tab === "habits" ? setAddOpen(true) : setNewRoutineOpen(true)}
      ...
      <div className="flex border-b-2 border-border flex-shrink-0">
        {(["habits", "routines"] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)}
            className={clsx("flex-1 py-3 text-[11px] font-bold uppercase tracking-widest transition-all border-b-[3px] -mb-[2px]",
        ...
        {tab === "habits" && (
          <>
            <section className="mt-6 mb-10">
              <h2 className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-4 bauhaus-section-rule">Today</h2>
                ...
                {habits.length === 0 ? (
                  ...
                  return (
                    <div key={habit.id}
                      ...
                      <motion.div onClick={() => toggleLog(habit.id!, todayStr)} initial={false}
                      ...
                      <span onClick={() => toggleLog(habit.id!, todayStr)}
                        className={`flex-1 font-bold cursor-pointer text-sm uppercase tracking-wide ${isDone ? "line-through text-muted-foreground" : ""}`}>
                        {habit.name}
                      </span>
                      <div className="flex items-center gap-1 px-2 py-1 bg-muted border border-border shrink-0">
                        <span className="text-[10px] font-mono font-bold">{getStreak(habit.id!)}</span>
                        <span className="text-[9px] uppercase text-muted-foreground font-bold">d</span>
                      </div>
                      <button onClick={() => setEditingHabit(habit)} className="w-7 h-7 border border-border flex items-center justify-center text-muted-foreground hover:bg-muted shrink-0">
                        <Pencil className="w-3.5 h-3.5" />
                      </button>
                      <button onClick={() => handleDelete(habit)} className="w-7 h-7 border border-destructive/30 flex items-center justify-center text-destructive hover:bg-destructive/10 shrink-0">
            ...
            {habits.length > 0 && (
              <section>
                <h2 className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-4 bauhaus-section-rule">30-Day Overview</h2>
                <div className="bg-card border-2 border-border p-4 overflow-x-auto no-scrollbar">
                  <div className="min-w-max space-y-3">
                    {habits.map((habit) => (
                      <div key={habit.id} className="flex items-center gap-3">
                        <span className="w-16 truncate text-[10px] font-bold text-right uppercase tracking-wide">{habit.name}</span>
                        <div className="flex gap-[2px]">
                          {last30Days.map((dateStr) => {
                            const isDone = allLogs.some((l) => l.habitId === habit.id && l.date === dateStr);
                            return <div key={dateStr} className="w-3 h-3" style={{ backgroundColor: isDone ? habit.color : "hsl(var(--muted))" }} title={dateStr} />;
                          })}
                        </div>
                      </div>
                    ))}
        ...
        {tab === "routines" && (
          <section className="mt-6">
            <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest mb-4">
              Pre-set time blocks — apply to timeline in one tap
            </p>
            {compositions.length === 0 ? (
              <div className="p-8 text-center border-2 border-dashed border-border bauhaus-hatch">
                <p className="font-bold uppercase tracking-wide text-sm text-muted-foreground">No routines yet</p>
                <p className="text-xs mt-1 text-muted-foreground/70">Tap + to create your first routine</p>
              </div>
            ) : (
              <div className="space-y-[2px]">
                  ...
                  return (
                    <div key={comp.id} className="bg-card border border-border">
                      <div className="flex items-center gap-3 px-3 py-3">
                        ...
                        <button onClick={() => applyRoutine(comp)}
                        ...
                        <button onClick={() => confirm(`Delete "${comp.name}"?`) && deleteComposition(comp.id!)}
                          className="w-7 h-7 border border-destructive/30 flex items-center justify-center text-destructive hover:bg-destructive/10 flex-shrink-0">
                          <Trash2 className="w-3.5 h-3.5" />
                        </button>
                      </div>
                      <div className="flex h-2 mx-3 mb-3 overflow-hidden gap-[1px]">
                        {comp.blocks.map((bl, i) => {
                          ...
                          return <div key={i} style={{ width: `${(bl.durationMinutes / totalMins) * 100}%`, backgroundColor: cat?.color ?? comp.color }} title={`${bl.label || cat?.name} · ${bl.durationMinutes}m`} />;
                        })}
                      </div>
                        ...
                        {comp.blocks.map((bl, i) => {
                          ...
                          return (
                            <div key={i} className="flex items-center gap-1 bg-background border border-border px-2 py-1">
                              ...
                              <span className="text-[9px] font-bold uppercase tracking-wide">{bl.label || cat?.name} · {bl.durationMinutes}m</span>
                            </div>
                          );
                        })}
                      </div>
      ...
      {editingHabit && <HabitForm title="Edit Habit" initial={editingHabit} onSave={handleEdit} onCancel={() => setEditingHabit(null)} />}
      {newRoutineOpen && <RoutineForm onSave={async (c) => { await addComposition(c); setNewRoutineOpen(false); }} onCancel={() => setNewRoutineOpen(false)} />}
    </div>
  );
}
